...
- **2025-12-22 XX:XX** - 线程池隔离策略不清晰，缺少隔离粒度设计和线程池配置
  - **错误上下文**：HystrixMethodInterceptor 和 CircuitBreakerManager 中，通过 `circuitBreakerManager.getExecutor(methodKey)` 获取线程池，用于线程隔离。
  - **错误描述**：线程池隔离策略不清晰，存在多个设计问题：1) **隔离粒度不明确**：没有明确说明是每个方法使用独立的线程池，还是多个方法共享一个线程池（服务级隔离）。如果是方法级隔离，会创建大量线程池导致资源浪费；如果是服务级隔离，应该提供 `groupKey` 来标识服务。2) **缺少线程池配置**：没有看到如何配置线程池参数（核心线程数、最大线程数、队列大小、拒绝策略等），这些参数对于线程隔离至关重要，不同的服务对线程池参数的需求不同（IO 密集型 vs CPU 密集型）。3) **缺少拒绝策略处理**：当线程池满了（队列也满了）时，应该如何处理？应该直接抛异常触发降级，还是记录为失败并触发熔断？
  - **严重性等级**：High
  - **错误分析**：违反了"线程池隔离需要明确隔离粒度和配置策略"的设计原则。线程池隔离不仅仅是"把任务提交到线程池"，还需要考虑隔离粒度（方法级 vs 服务级）、线程池配置（如何平衡资源利用和隔离效果）、拒绝策略（如何处理线程池满的情况）等问题。
  - **可能后果**：1) **资源浪费**：如果每个方法都创建独立线程池，会创建大量线程池，浪费内存和线程资源；2) **隔离效果差**：如果多个服务共享一个线程池，一个服务的异常可能影响其他服务；3) **配置不灵活**：无法根据不同服务的特点配置线程池参数，可能导致性能问题；4) **拒绝处理不当**：线程池满了后的处理策略不明确，可能导致请求失败或熔断判断不准确。
  - **修复建议**：1) **明确隔离粒度**：参考 Hystrix 的设计，使用 `commandKey`（方法级）和 `groupKey`（服务级）两层标识，默认按 `groupKey` 分配线程池；2) **提供线程池配置**：在 `@HystrixCommand` 注解或配置文件中提供线程池参数配置；3) **实现拒绝策略处理**：当线程池满了，应该记录为失败并触发降级。引导用户思考：Hystrix 中的线程池隔离粒度是什么？是每个方法一个线程池，还是每个服务（多个方法）共享一个线程池？为什么要这样设计？
  - **相关上下文**：用户询问线程隔离的实现问题，代码审查时发现线程池隔离策略不清晰。
