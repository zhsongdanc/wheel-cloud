...
### 并发编程
- **2025-01-XX XX:XX** - 用户对并发场景下的顺序保证设计思路不熟悉，不清楚如何在并发执行时保证顺序
  - **知识点说明**：在并发设计中，常见的思路是"哪个先完成就先处理哪个"，这样可以最大化并发度，不阻塞。但在需要保证顺序的场景下（如分片上传需要按 PartNumber 顺序），这种思路不合适。**正确的思路**：1) **并发查询**：多个批次可以并发查询，提高效率；2) **顺序上传**：但上传必须按顺序（批次1 → 分片1，批次2 → 分片2...）；3) **等待机制**：如果批次2先完成，需要等待批次1完成并上传后，才能上传批次2。**实现方式**：使用锁、队列或其他同步机制保证上传顺序。**关键点**：在并发场景下，需要区分"可以并发的部分"和"必须顺序的部分"，并发部分可以最大化效率，顺序部分必须保证顺序。
  - **相关上下文**：用户询问"为什么你会有按照完成顺序上传这种思路，我甚至没想到还有这种思路"，在讨论分片上传的设计时。
